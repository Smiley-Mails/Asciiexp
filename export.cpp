...
void AsciiExp::ExportLightSettings(LightState* ls, GenLight* light, TimeValue t, int indentLevel)
{
	...
	_ftprintf(pStream, _T("%s\t\t%s %s\n"), indent.data(), ID_LIGHT_TDIST, Format(light->GetTDist(t, iv)));
	_ftprintf(pStream, _T("%s\t\t%s %s\n"), indent.data(), ID_LIGHT_MAPBIAS, Format(light->GetMapBias(t, iv)));
	_ftprintf(pStream, _T("%s\t\t%s %s\n"), indent.data(), ID_LIGHT_MAPRANGE, Format(light->GetMapRange(t, iv)));
	_ftprintf(pStream, _T("%s\t\t%s %s\n"), indent.data(), ID_LIGHT_MAPSIZE, Format(light->GetMapSize(t, iv)));
	_ftprintf(pStream, _T("%s\t\t%s %s\n"), indent.data(), ID_LIGHT_RAYBIAS, Format(light->GetRayBias(t, iv)));
...
//		_ftprintf(pStream, _T("%s\t\t%s %2d: [ %s]\n"), indent.data(), ID_MESH_VERTEX, i, Format(v));
//		_ftprintf(pStream, _T("%s\t\t%s %2d: [ %-10s%-10s%-10s ]\n"),
		_ftprintf(pStream, _T("%s\t\t%s %2d: [%8s %8s %8s ]\n"),
			indent.data(),
			ID_MESH_VERTEX,
			i,
			Format(v.x).data(),
	...
	//	_ftprintf(pStream, _T("%s\t\t%s %4d:    A: %4d B: %4d C: %4d AB: %4d BC: %4d CA: %4d"),
		_ftprintf(pStream, _T("%s\t\t%s %4d: [%2d,%2d,%2d]  AB:%d BC:%d CA:%d"),
			indent.data(),
			ID_MESH_FACE, i,
			mesh->faces[i].v[vx1],
			mesh->faces[i].v[vx2],
...
//			_ftprintf(pStream, _T("%s\t%s %d\n"),indent.data(), ID_MESH_NUMTVFACES, mesh->getNumFaces());
	...
	//			_ftprintf(pStream, _T("%s\t\t%s %d\t%d\t%d\t%d\n"),
				_ftprintf(pStream, _T("%s\t\t%s %3d: [%2d,%2d,%2d]  AB:%d BC:%d CA:%d\n"),
					indent.data(),
					ID_MESH_TFACE, i,
					mesh->tvFace[i].t[vx1],
					mesh->tvFace[i].t[vx2],
